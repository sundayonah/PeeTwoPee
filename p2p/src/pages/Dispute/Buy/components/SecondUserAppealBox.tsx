...
import { UPDATESECAPERSONRES } from "../../gql/mutations";
import { useActiveWeb3React } from "../../../../utils/hooks";
import { responseType } from "./UserAppealBox";
...
const SecondUserAppealBox = ({
 ...
 tradeType,
  ...
  // const { loading: loadTrx, tradeInfo, fromInfo } = useFetchTransactionByID(transactionId ?? "");
 ...
 const newAppealProofObj = {
  ...
  const [addSecRes, { data, loading, error }] = useMutation(
    UPDATESECAPERSONRES,
    {
      variables: {
        params: {
          ...
          newAppealProof:  newImage    
         },
      },
    }
  );

  const [addSecResWithoutProof, { data : noPoofData, loading : noProofLoad, error: noProofError }] = useMutation(
    UPDATESECAPERSONRES,
    {
      variables: {
        params: {
...
const handleAddReasonToAppeal = () => {
  ...
  useEffect(() => {
    if (data?.addSecondPersonResponse?.status === true || noPoofData?.addSecondPersonResponse?.status) {
      ...
      >
        ...
        >
          <Text fontSize={16} color={activeColour}>
            ...
            <span style={{ color: inactiveColour }}>
              ({tradeType})
            </span>
          </Text>
        ...
        <Box w={"full"} fontSize={14} p={4}>
          ...
          <Flex justifyContent="space-between" alignItems={"center"}>
            <Text color={inactiveColour} mb={3} mt={12}>
              {t('att')}
            </Text>
        ...
        <Box w={"full"} p={4}>
         ...
         { newImage ?
         <Text>{newImage.name + '.' + newImage.extension}</Text>
         :
         <Square
            cursor={"pointer"}
            onClick={() => {
...
export default SecondUserAppealBox;
