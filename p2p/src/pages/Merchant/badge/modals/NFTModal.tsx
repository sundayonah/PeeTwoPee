...
const StakingSucess = ({
  ...
  const { metadata, loading, newUri } = useNFTFetch(
    accessWallet,
    setAccessWallet,
    selectedNftCollection,
    tokenIds
  ...
  const { formattedTokenUri } = useFormatURI(tokenIds, newUri);
  ...
  const [updateRank, { loading: load, error, data }] = useMutation(
    UPDATE_RANK,
    {
      variables: {
        chain: `${chainId}`,
                      ...
                      metadata.map((item) => {
                        return (
                          <Box
                            ...
                            border={"1px"}
                            borderColor={
                              item.id === tokenId ? borderColor : "transparent"
                            }
                            p={2}
                          >
                            <Image
