...
import ReasonsBox from "../../../Dispute/Buy/components/ReasonsBox";
import { LostBox, responseType, WinBox } from "../../../Dispute/Buy/components/UserAppealBox";
      ...
      >
        ...
        >
          <Text fontSize={16} color={activeColour}>
            ...
            <span style={{ color: "#319EF6" }}>{appealUser}</span>{" "}
            <span style={{ color: inactiveColour }}>({traderType})</span>
          </Text>

          <Spacer />
          {disputeStatus == "VOTINGENDED" && (isWinner ? <WinBox/> : <LostBox/>)}
          <Box p={1.5} rounded="sm" backgroundColor={"#EBF6FE"}>
            <Text color={"#319EF6"}>{votes} votes in favour</Text>
          </Box>
        </Flex>
        ...
        <Box w={'full'} fontSize={14} p={4}>
          ...
          <Text color={activeColour}> {apealReason}</Text>
          ...
          >
            <Box
              ...
              maxH="176px"
            >
              <Text color={activeColour}>{appealMessage}</Text>
            </Box>
          </Box>
          ...
          {appealproofs?.length >= 1
            ? appealproofs.map((appeal, index) => (
                <Box
                  key={index}
                  p={3}
                  border="0.1px solid"
                ...
                >
                  <Flex justifyContent="space-between" alignItems={"center"}>
                    <HStack>
                      ...
                      <Text>
                        {appeal.name}.{appeal.extension}{" "}
                      </Text>
                    </HStack>
                    <Link isExternal href={appeal.uri} color={"#319EF6"}>
                      View
                    </Link>
                  </Flex>
                </Box>
